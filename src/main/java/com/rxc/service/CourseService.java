...
public class CourseService {
    ...
    public boolean findCourseFromStudent(String selectItem, String inputInfor) {  //学生查寻课程，可以根据名称，教师
        ...
        if (selectItem.equals("1")) {         //根据的课程名称查找
            ...
            List<List<Teacher>> coutealist = new ArrayList<>();
            ...
            List<Teacher> teacherlist = teacherDao.findTeacherByname(inputInfor);       //得到教师名称包含inputInfor的所有教师
            List<List<Course>> teacoulist = new ArrayList<>();
    ...
    public void getAllTC() {           //学生点击了查询所有可选课程的按钮
        ...
        for (TC tc : tclist) {
            ...
            courselist.add(cou);
            Teacher tea = teacherDao.findTeacher(tno);
            teacherlist.add(tea);
        }
        request.setAttribute("courselist", courselist);
    ...
    public boolean studentChooseCourse(String cno, String tno) {            //学生选课
        ...
        if (gradeDao.insertIntoGrade(tcid, sno)) {
            request.setAttribute("message", "选课成功");
            return true;
        } else {
    ...
    public boolean studentQuitCourse(String cno, String tno) {               //学生退课
        ...
        String sno = ((Student) request.getSession().getAttribute("stu")).getSno().trim();
        int tcid = tcDao.getTcid(cno, tno);     //先得到这门课的选课记录编号
        if (gradeDao.deleteStudentCourse(tcid, sno)) {
            request.setAttribute("message", "退选成功");
            return true;
        } else {
    ...
    public void teacherSeeDetail(String cname) {             //教师查看他教的某门课的人数情况，以便录入修改成绩
        ...
        List<Grade> gradelist = gradeDao.getAllSnoByTcid(tcid);       //获取上这门课的所有学生
        List<Student> stulist = new ArrayList<>();
        for (Grade grade : gradelist) {
            Student stu = studentDao.findStudent(grade.getSno());         //根据 学号一个一个查学生
            stulist.add(stu);
        }
    ...
    public boolean teacherUpdateScore(int tcid, String sno, double cscore) {       //教师保存一项成绩,可以是录入，也可以是修改
        HttpServletRequest request = ServletActionContext.getRequest();
        if (gradeDao.teacherUpdateScore(tcid, sno, cscore)) {
    ...
    public void studentSeeScore() {                    //学生查询课程成绩
        ...
        List<Teacher> teacherlist = new ArrayList<>();
        List<Course> courselist = new ArrayList<>();
        List<Grade> gradelist = gradeDao.studentSeeScore(stu.getSno());        //根据学号得到他的课程成绩
        for (Grade grade : gradelist) {
            TC tc = tcDao.getTcByTcid(grade.getTcid());          //得到这门课的任课情况
            Teacher tea = teacherDao.findTeacher(tc.getTno());
            teacherlist.add(tea);
            Course cou = courseDao.getCourseByCno(tc.getCno());
            courselist.add(cou);
        }
